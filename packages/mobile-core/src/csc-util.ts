...
export class CSCUtil {
    ...
    static decodeMemos(memos: Array<CasinocoinMemo>): Array<Memo> | undefined {
        function removeUndefined(obj: any): Object {
    ...
    static encodeMemo(inputMemo: Memo): CasinocoinMemo {
        ...
        function stringToHex(string: string | undefined): string | undefined {
    ...
    static decodeInvoiceID(hex: string): string {
        ...
        function removePadStart(string: string, padString: string) {
        ...
        const unpaddedString: string = removePadStart(hex, '00');
    ...
    static encodeInvoiceID(encodeString: string): string {
        ...
        function padStart(inputString: string, padString: string) {
            let targetLength = 64;
        ...
        // encode
        let encoded:string = encodeString ? (Buffer.from(encodeString.substring(0, 32), 'utf8')).toString('hex').toUpperCase() : '';
    ...
    static convertStringVersionToNumber(version: string) {
        // remove points
    ...
    static decodeCSCQRCodeURI(input: string): CSCURI {
        ...
        let address, inputToken, amount, destinationTag, label;
        params.forEach( value => {
            if (value.startsWith('to=')) {
                address = value.substr(value.indexOf('=') + 1);
            } else if (value.startsWith('token=')) {
                inputToken = value.substr(value.indexOf('=') + 1);
